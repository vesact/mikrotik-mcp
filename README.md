...
Create a KeePass vault (e.g. `config/vault.kdbx`) with one entry per device inside a group (default: `Mikrotik-CHR`):
...
> "Show DHCP leases on branch-router-01."
...
| `KEEPASS_GROUP` | `Mikrotik-CHR` | Vault group to enumerate devices from |
