...
describe('parseDetailRecords', () => {
  ...
  it('parses single record with quoted and unquoted values', () => {
    const raw =
      'Flags: X - DISABLED\r\n 0    name="routeros" version="7.22.1" build-time=2026-03-23 size=11.7MiB \r\n\r\n';
    const records = parseDetailRecords(raw);
    expect(records).toHaveLength(1);
    expect(records[0]).toEqual({
      name: 'routeros',
      version: '7.22.1',
      'build-time': '2026-03-23',
      size: '11.7MiB',
    });
  });
...
describe('parseRouterboard', () => {
  it('parses routerboard output', () => {
    const raw = [
      ...
      '        board-name: hEX        \r',
      '             model: E50UG      \r',
      '     serial-number: HH90AA40QJ8\r',
      '     firmware-type: en7562     \r',
      '  current-firmware: 7.15.3     \r',
    ...
    expect(parseRouterboard(raw)).toEqual({
      ...
      boardName: 'hEX',
      model: 'E50UG',
      serialNumber: 'HH90AA40QJ8',
      firmwareType: 'en7562',
      currentFirmware: '7.15.3',
