...
project_name: 'mikrotik-mcp'
user_name: 'Actemium Schweiz AG'
date: '2026-04-19'
---
...
- **Group:** Configurable via `KEEPASS_GROUP` env var; default `Mikrotik-CHR`
- **Field mapping:**
  - Title → device ID (e.g. `MikroTik85`)
...
| `KEEPASS_GROUP` | No | `Mikrotik-CHR` | KeePass group to enumerate devices from |
...
Fleet-wide dogfood (90 CHR devices, all 51 read commands) revealed that per-tool parser functions silently drop fields. The architecture's original pattern — dedicated `parseXxx()` functions cherry-picking known fields — is structurally unable to pass through version-specific or newly-added RouterOS fields, and is the root cause of 10 parsing bugs.
...
tests/fixtures/
  firewall-filter/
    v7.13.2-chr.txt     # raw CLI output
    v7.14.3-chr.txt
    v7.22.1-chr.txt
  ip-address/
    v7.13.2-chr.txt
...
Fleet-wide audit confirmed all 83 reachable devices (out of 87) have the RouterOS `www` service enabled on port 82 (HTTP). RouterOS 7.1+ exposes a REST API at `/rest/...` that returns native JSON, eliminating the need for CLI text parsing — the root cause of all 10 parsing bugs (P1–P10) and the motivation for Epic 11 stories 11-1, 11-3, and 11-6.
...
- Filtering via query params: `?network=10.0.0.0&dynamic=true`
...
**Actemium fleet configuration:**
```env
ROUTEROS_TRANSPORT=rest
ROUTEROS_REST_PORT=82
ROUTEROS_REST_SCHEME=http
```
...
- URL → hostname/IP
- Port → **currently hardcoded to 22** — becomes unused when REST is active. The REST port is fleet-wide via `ROUTEROS_REST_PORT` env var (not per-device) since the fleet is uniform.

**Future option:** If per-device port variation is needed, the KeePass Port field could be used. Not needed for current fleet.
...
// SSH:  /ping address=10.0.0.1 count=4
// REST: POST /rest/ping  body: {"address":"10.0.0.1","count":"4"}
...
- **HTTP (port 82) vs HTTPS (port 443):** The fleet currently uses HTTP on port 82 with `www-ssl` disabled. HTTP Basic Auth over unencrypted HTTP exposes credentials to passive eavesdropping on the network. This is acceptable only if the management network is trusted/segmented. Recommend migrating fleet to HTTPS (`www-ssl`) with self-signed certificates as a follow-up.
