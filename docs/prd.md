---
stepsCompleted: ['step-01-init', 'step-02-discovery', 'step-02b-vision', 'step-02c-executive-summary', 'step-03-success', 'step-04-journeys', 'step-05-domain', 'step-06-innovation', 'step-07-project-type', 'step-08-scoping', 'step-09-functional', 'step-10-nfr', 'step-11-polish']
inputDocuments: ['product-brief-mikrotik-mcp.md', 'product-brief-mikrotik-mcp-distillate.md']
workflowType: 'prd'
classification:
...
`mikrotik-mcp` is a Model Context Protocol (MCP) server that gives AI assistants and automated agents structured, auditable access to MikroTik RouterOS 7+ devices over SSH. Built for Actemium's network engineering team managing ~100 MikroTik devices, it eliminates the manual per-device workflow that currently requires Winbox or raw SSH sessions for every operational task.

Engineers interact via natural language — *"show all interfaces with errors across the fleet"*, *"reconfigure RADIUS on site-B devices"*, *"check system logs post-incident"* — and `mikrotik-mcp` executes those operations across the fleet, returning structured per-device results. Credentials are resolved at runtime from a KeePass `.kdbx` vault; no password ever appears in LLM context or logs.

Built in TypeScript, deployable via Docker, and covered by integration tests against real RouterOS hardware.

### What Makes This Special

Every one of the 31 existing MikroTik MCP servers on GitHub shares three limitations: single-device only, plaintext credentials in config, and no real-device test coverage. `mikrotik-mcp` is the first to solve all three simultaneously.
...
- Network engineers at Actemium complete fleet-wide operational tasks (audit, reconfigure, diagnose) via their AI assistant without opening Winbox or a terminal
...
- All confirmed Winbox sections are implemented and functional against the production fleet of ~100 devices
- The tool is deployed and in regular use within Actemium within 30 days of completion
- If open-sourced: community adoption signals — GitHub stars, issues, forks, and reports of production use at other organisations
...
**Marco** is a network engineer at Actemium. Monday morning, a weekend incident report suggests some devices may have an open DNS resolver. He opens Claude (with mikrotik-mcp configured) and types: *"Check all devices for open DNS resolvers and give me a per-site report."*

`mikrotik-mcp` fans out the query across all 100 devices. Credentials are fetched silently from the KeePass vault — Marco never typed a password, never opened Winbox. The assistant returns a structured table: 3 devices in site-C have the issue, 97 are clean. Marco asks it to fix the 3 affected devices. The audit log captures every command with timestamp, device, and result.
...
**Sophie** needs to update the RADIUS server address on all site-B devices following an infrastructure migration. She asks: *"Update the RADIUS server on site-B devices to 192.0.2.5."*

`mikrotik-mcp` resolves the "site-B" group, executes the RADIUS update on each device, and returns a per-device success/failure summary. One device was unreachable — reported cleanly. Sophie logs a ticket.
...
**Marco** is paged at 2am. He asks: *"Show me the last 50 log entries and interface status for router-site-A-01."*
...
**Before:** VPN in, open Winbox, navigate manually — slow and stressful at 2am. **After:** Results in seconds, even in degraded device state.
...
- 31 MikroTik MCP servers exist on GitHub (April 2026); all are single-device, plaintext-credential, and untested against real hardware
...
- Internal deployment at Actemium on the ~100-device fleet provides live production validation
...
- **MVP:** Sequential per-device SSH execution within fan-out — sufficient for ~100 devices; documented as a known limitation
...
- **NFR-PERF-1:** MVP uses sequential per-device SSH execution within fan-out — acceptable for ~100 devices; documented as a known limitation
