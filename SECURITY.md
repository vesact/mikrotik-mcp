...
- **Set `MCP_ALLOWED_HOSTS` / `MCP_ALLOWED_ORIGINS` in HTTP mode.** Without them, a page loaded in any browser that can reach the listener can drive the full tool set through DNS rebinding. Both lists are unset by default and the server warns at startup.
